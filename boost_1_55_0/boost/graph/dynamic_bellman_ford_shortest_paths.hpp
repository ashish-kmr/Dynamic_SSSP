...
#ifndef DYNAMIC_BOOST_GRAPH_BELLMAN_FORD_SHORTEST_PATHS_HPP
#define DYNAMIC_BOOST_GRAPH_BELLMAN_FORD_SHORTEST_PATHS_HPP
...
namespace boost {
  ...
  { 
    ...
    typedef typename GTraits::vertex_descriptor Vertex, v_init, v_end;
    ...
    typedef typename GTraits::in_edge_iterator in_edg;
    in_edg i;
    ...
    for (Size k = 0; k < N; ++k) { 
      ...
      for (vtx v_i= vtx_itr.first; v_i != vtx_itr.second; ++v_i) {
        for (e_it=in_edges(*v_i,g); e_it.first != e_it.second; ++e_it.first){
	i=e_it.first;	
        v.examine_edge(*i, g);
...
*/
  ...
  {                                
    ...
    return dynamic_bellman_ford_shortest_paths(g, N, params);
  }
